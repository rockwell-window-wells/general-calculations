# -*- coding: utf-8 -*-
"""
Created on Fri Jul 21 10:44:36 2023
...
df = pd.read_excel("Resin Trash Correlation.xlsx",sheet_name="Sheet4")
...
df_old = df[df["Reduced or Old Resin Amount"]=="Old"]
df_new = df[df["Reduced or Old Resin Amount"]=="Reduced"]
...
df["Adjusted Reduced or Old"] = df.apply(lambda x: switch_flag(x["Reduced or Old Resin Amount"], x["Diff"], x["Max Diff"]), axis=1)

# Filter out cases where the part size doesn't remotely match the resin written down
df = df[np.abs(df["Diff"]) <= df["Max Diff"]]
df_old = df[df["Adjusted Reduced or Old"]=="Old"]
df_new = df[df["Adjusted Reduced or Old"]=="Reduced"]
...
ctab = pd.crosstab(df["Adjusted Reduced or Old"], df["Good/Repair/Trash"])
# ctab = pd.crosstab(df["Reduced or Old Resin Amount"], df["Good/Repair/Trash"])
...
binary_cols = pd.get_dummies(df72["Good/Repair/Trash"])
df72 = pd.concat((df72, binary_cols["Good"]), axis=1)
quality_list = list(binary_cols.columns)
if "Repair" in quality_list:
    df72 = pd.concat((df72, binary_cols["Repair"]), axis=1)
if "Trash" in quality_list:
...
binary_cols = pd.get_dummies(df["Good/Repair/Trash"])
quality_list = list(binary_cols.columns)
...
ctab = pd.crosstab(df["Mold"], df["Good/Repair/Trash"])
stat, p, dof, expected = stats.chi2_contingency(ctab)
alpha = 0.05
...
# Part color against repair or trash
ctab = pd.crosstab(df["Part Color"], df["Good/Repair/Trash"])
stat, p, dof, expected = stats.chi2_contingency(ctab)
alpha = 0.05
print("\nRelationship between part color and trash or repair")
print("p value is " + str(p))
if p <= alpha:
    print("Dependent (reject null hypothesis)")
else:
